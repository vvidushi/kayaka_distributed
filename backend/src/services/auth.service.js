...
export const register = async (userData) => {
  ...
  // If SSN is provided during registration, validate it's not already used with a different email
  if (ssn) {
    const { getPostgresPool } = await import('../config/database.js');
    const pool = getPostgresPool();
    try {
      const existingSsnResult = await pool.query(
        'SELECT id, email FROM users WHERE ssn = $1',
        [ssn]
      );

      if (existingSsnResult.rows.length > 0) {
        const existingSsnUser = existingSsnResult.rows[0];
        throw new Error(`This SSN is already associated with email: ${existingSsnUser.email}. Each SSN can only be linked to one account.`);
      }
    } catch (ssnError) {
      // If it's our custom error, re-throw it
      if (ssnError.message.includes('already associated')) {
        throw ssnError;
      }
      // Otherwise, log and continue (might be a connection issue)
      logger.warn('Error checking SSN during registration:', ssnError.message);
    }
  }
