...
import { updateUser as updateUserAction, clearAuth } from '../store/slices/authSlice';
import { uploadProfileImage } from '../services/image.service';
import { useToast } from '../hooks/useToast';
import { FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt, FaImage, FaSave, FaSpinner, FaBan, FaCheckCircle } from 'react-icons/fa';
...
const ProfilePage = () => {
  ...
  const deactivateAccountMutation = useMutation({
    mutationFn: () => usersApi.deactivateUser(currentUser.id),
    onSuccess: () => {
      toast.showSuccess('Account deactivated. You will be signed out.');
      dispatch(clearAuth());
      setTimeout(() => {
        navigate('/login');
      }, 1500);
  ...
  const handleDeactivateAccount = () => {
    const confirmed = window.confirm(
      'Are you sure you want to deactivate your account? You will not be able to log in until an admin reactivates it. This action can be reversed by contacting support.'
    );
    if (!confirmed) return;
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-base-content/70 mb-3">
                Temporarily disable your account. You will not be able to log in until an admin reactivates it.
              </p>
              <button
                type="button"
                className="btn btn-warning btn-outline"
                onClick={handleDeactivateAccount}
                disabled={deactivateAccountMutation.isLoading || currentUser?.role === 'admin'}
              >
            ...
            <div>
              <h4 className="font-semibold mb-2 text-error">Delete Account</h4>
              <p className="text-sm text-base-content/70 mb-3">
